...
function createSection(cur,gen,cb) {
  ...
  if(gen.config.get('app') === 'electron') {
    fs.stat('src/app', function(err, stat) {
      if(err) {
        fs.mkdirSync('src/app');
      }
      fs.createReadStream(path.join(process.cwd(), '../../templates/app/electron.index.js')).pipe(fs.createWriteStream('src/app/electron.index.js'));
      fs.createReadStream(path.join(process.cwd(), '../../templates/app/index.html')).pipe(fs.createWriteStream('src/app/index.html'));
    });
  } 
