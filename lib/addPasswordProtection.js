...
function createHtPasswd(location, password) {
  ...
  fs.writeFile(path.join(location,'.htpasswd'), htpasswdData, function(err) {
    if (err) throw err;
    console.log("Http password successfully created");
  });
}
...
function modifyHtAccess(location) {
  ...
  prependFile(path.join(location,'.htaccess'), htaccessData, function(err) {
    if (err) throw err;
    console.log("Added password configuration to HTTP Access");
  });
}